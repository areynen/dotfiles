...
SAVE LAYOUT
DELETE LAYOUT" | dmenu -p "Select action")
  ...
  LAYOUT_NAME=$(echo "$LAYOUT_NAMES" | dmenu -p "Select layout (you may type new name when creating)" | sed 's/\s/_/g') # ask for selection
...
if [[ "$ACTION" = "LOAD LAYOUT" ]]; then
  ...
  for window in $WINDOWS; do
    ...
    # however, the line is not there when calling with dmenu from i3
...
SPECIFIC (CHOOSE)
MATCH ANY" | dmenu -p "How to identify windows? (xprop style)")
  ...
  if [[ "$CRITERION" = "default" ]]; then
    ...
    while true; do
      ...
      if [ ! -z "$LINE_NUM" ]; then
        ...
        SELECTED_OPTION=$(cat -n $LAYOUT_FILE | sed -n "${LINE_NUM},$(expr $LINE_NUM + 2)p" | awk '{$2="";print $0}' | dmenu -p "Choose the matching method for${NAME%,}" | awk '{print $1}')
